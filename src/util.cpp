...
const char * const BITCOIN_CONF_FILENAME = "lexoncoin.conf";
const char * const BITCOIN_PID_FILENAME = "lexoncoin.pid";
...
#else
    const char* pszModule = "lexoncoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Lexoncoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Lexoncoin";
#else
    // Unix
    return pathRet / ".lexoncoin";
#endif
#endif
}
